...
def pandoc_to_omml(latex_formula):
    """
    Fallback: Convert LaTeX to OMML using pandoc by creating a temp docx.
    """
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Run pandoc
            subprocess.run(
                ["pandoc", tex_file, "-f", "latex", "-t", "docx", "-o", docx_file],
                capture_output=True,
                timeout=10
            )
            ...
            if os.path.exists(docx_file):
                with zipfile.ZipFile(docx_file, 'r') as z:
                    ...
                    # Search for oMath or oMathPara
                    math_match = re.search(r'<m:oMathPara.*?</m:oMathPara>', xml_content, re.DOTALL)
                    if not math_match:
                        math_match = re.search(r'<m:oMath.*?</m:oMath>', xml_content, re.DOTALL)
                    
                    if math_match:
                        omml = math_match.group(0)
        ...
        print(f"Pandoc fallback failed: {e}")
        return None


def latex_to_omml(latex_formula):
    """
    Convert LaTeX formula to OMML (Office Math Markup Language) using texmath (primary) or pandoc (fallback).
    ...
    try:
        ...
        # (This script doesn't have clean_latex_delimiters defined, but we can use the same logic if needed)
        
        # Search for texmath in PATH
        import shutil
        texmath_path = shutil.which("texmath")
        ...
        if texmath_path:
            ...
            if result.returncode == 0:
                ...
                print(f"Error converting formula with texmath: {result.stderr.decode('utf-8')}")
        
        # Try pandoc fallback
        return pandoc_to_omml(latex_formula)
